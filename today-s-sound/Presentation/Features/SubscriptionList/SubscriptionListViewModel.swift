    ...
    .sink(
      receiveCompletion: { [weak self] completion in
        ...
        case .failure(let error):
          ...
          print("❌ 구독 목록 조회 실패: \(self.errorMessage ?? "")")
        }
      },
      receiveValue: { [weak self] response in
        ...
        self.subscriptions.append(contentsOf: newItems)
        ...
        if newItems.count < self.pageSize {
          ...
          print("🏁 마지막 페이지 도달: 받은 개수(\(newItems.count)) < 예상(\(self.pageSize))")
        }
        
        print("✅ 구독 목록 조회 성공: \(newItems.count)개 추가 (전체: \(self.subscriptions.count)개)")
      }
    )
